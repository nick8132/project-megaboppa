	...
	if(machine){
		...
		function sp() {
			if(gamestart.ended == true){
				menu();
				setTimeout(function() {underline(1);window.addEventListener("keydown", keys, false);ifonmobile=true}, 300);
			}else{
...
function room() {
	...
	var canvas = document.getElementById("myCanvas");
    var g2d = canvas.getContext("2d");
    var img = document.getElementById("room_bg");
    g2d.drawImage(img, 0, 0, canvas.width, canvas.height);
    if (localStorage.getItem("room") == "") {
    	console.log("you need to go to the shop to buy some shelfs and some light")
    }else if (localStorage.getItem("room") == "") {
    	console.log("you need light and shelfs")
    }else if(localStorage.getItem("room") == ""){
    	console.log("")
    }else if(localStorage.getItem("room") == ""){
